...
function init() {
    ...
    document.getElementById('selected-point').textContent = 'Loading model... Please wait';
    ...
    setTimeout(() => {
        try {
            ...
            document.getElementById('selected-point').textContent = 'Error loading model. See console for details.';
        }
    }, 100);
}
...
function loadModelFromBase64() {
    ...
    document.getElementById('selected-point').textContent = 'Processing model... Please be patient';
    ...
    if (modelData.textureData) {
        ...
        img.src = modelData.textureData;
        
        // Important: Direct use of the data URL prevents CORS issues
        texture = textureLoader.load(
            modelData.textureData, // Use the data URL directly
            // onLoad callback
            function(loadedTexture) {
                ...
                loadObjWithoutFile(modelData.objData, texture);
            },
            // onProgress callback
            function(xhr) {
                const percent = Math.round(xhr.loaded / xhr.total * 100);
                console.log(`Texture: ${percent}% loaded`);
                document.getElementById('selected-point').textContent = `Loading texture: ${percent}%`;
            },
            // onError callback
            function(error) {
                console.error('Error loading texture:', error);
                // Load the model without texture if there's an error
                loadObjWithoutFile(modelData.objData, null);
            }
        );
    } else {
...
function loadObjWithoutFile(objDataUrl, texture) {
    ...
    try {
        ...
        document.getElementById('selected-point').textContent = 'Model loaded.';
    } catch (error) {
        console.error('Error parsing OBJ data:', error);
        document.getElementById('selected-point').textContent = 'Error loading model';
    }
}
...
function loadPoints() {
    ...
    Object.entries(pointsData).forEach(([id, coordinates]) => {
...
function playAudio(contactId) {
    ...
    if (audioData && audioData[contactId]) {
        console.log(`Playing audio for ${contactId}`);
        
        // Create a new audio element
        const audio = new Audio(audioData[contactId]);
        
        // Add event listeners
        audio.addEventListener('play', () => {
            console.log(`Audio for ${contactId} started playing`);
            document.getElementById('selected-point').textContent = `Playing audio for: ${contactId}...`;
        });
        
        audio.addEventListener('ended', () => {
            console.log(`Audio for ${contactId} finished playing`);
            document.getElementById('selected-point').textContent = `Selected Point: ${contactId}`;
            currentAudio = null;
        });
        
        audio.addEventListener('error', (e) => {
            console.error(`Error playing audio for ${contactId}:`, e);
            document.getElementById('selected-point').textContent = `Error playing audio for: ${contactId}`;
            currentAudio = null;
        });
        ...
        document.getElementById('selected-point').textContent = `No audio available for: ${contactId}`;
    }
}
    ...
    if (intersects.length > 0) {
        ...
        for (let i = 0; i < intersects.length; i++) {
            ...
            if (pointId) {
                ...
                document.getElementById('selected-point').textContent = `Selected Point: ${pointId}`;
...
// Handle splash screen with fade-out effect
function handleSplashScreen() {
    const splashScreen = document.getElementById('splash-screen');
    
    // Show splash screen for 1.25 seconds, then fade out
    setTimeout(() => {
        ...
        }, 500); // Same as transition duration in CSS
    }, 1250); // Display for 1.25 seconds
}

// Show volume warning dialog
function showVolumeWarning() {
    const volumeWarning = document.getElementById('volume-warning');
    const okButton = document.getElementById('volume-warning-ok');
...
// Handle the splash screen before starting the application
handleSplashScreen();
...
function loadScript(url) {
    return new Promise((resolve, reject) => {
        ...
        const existingScript = document.getElementById('model-data-script');
        if (existingScript) {
            existingScript.remove();
        }
        
        // Before loading a new data file, check if modelData exists and clean it up
        if (typeof modelData !== 'undefined') {
            // Store the data in a temporary window property if needed
            window._tempModelData = null;
            
            // Remove the existing modelData from global scope to prevent redeclaration errors
            try {
                window.modelData = undefined;
                // Use eval with try-catch as a last resort to clean up const variables
                // This isn't ideal but helps with the immediate issue
                eval('var modelData = undefined; var pointsData = undefined; var audioData = undefined;');
            } catch (e) {
                console.log('Variable cleanup attempted');
            }
        }
        
        // Create a new script element
        const script = document.createElement('script');
        script.id = 'model-data-script';
        script.src = url;
        script.async = true;
...
async function switchModel(modelFile) {
    try {
        // Show the splash screen again during model loading
        const splashScreen = document.getElementById('splash-screen');
        splashScreen.style.display = 'flex';
        splashScreen.style.opacity = '1';
        
        // Update status
        document.getElementById('selected-point').textContent = `Loading model from ${modelFile}...`;
        
        // We need to reload the page to properly handle const redeclarations
        // Add a query parameter to specify which model to load on reload
        const url = new URL(window.location.href);
        url.searchParams.set('model', modelFile);
        window.location.href = url.toString();
        
        // The page will reload, so we stop execution here
        return true;
    
    } catch (error) {
        console.error('Error switching models:', error);
        document.getElementById('selected-point').textContent = `Error loading model: ${error.message}`;
        return false;
    }
}

// Initialize event listeners for model selector
